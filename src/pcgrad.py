...
def pc_grad_update(gradient_list):
    ...
    def flatten_and_store_dims(grad_task):
        output = []
        grad_dim = []
        # TODO(speedup): convert to map since they are faster
        for param_grad in grad_task:
    ...
    def project_gradients(grad_task):
        ...
        for k in range(num_tasks):  # TODO(speedup): convert to map since they are faster
            ...
            conflict_gradient_candidate, _ = flatten_and_store_dims(
                conflict_gradient_candidate)

            inner_product = torch.dot(torch.flatten(
                grad_task), torch.flatten(conflict_gradient_candidate))
            ...
            proj_direction = inner_product / \
                torch.norm(conflict_gradient_candidate)**2
            ...
            grad_task = grad_task - \
                min(proj_direction, 0.) * conflict_gradient_candidate
